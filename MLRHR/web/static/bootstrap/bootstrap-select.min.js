/*
 * ! bootstrap-select v1.1.1 http://silviomoreto.github.io/bootstrap-select/
 * 
...
!function(b) {
	...
	a.prototype = {
		...
		createLi : function() {
			...
			this.$element.find("option").each(function(h) {
				...
				var l = j.data("subtext") !== undefined
						? '<small class="muted">' + j.data("subtext")
								+ "</small>"
						: "";
				var k = j.data("icon") !== undefined ? '<i class="glyphicon '
						+ j.data("icon") + '"></i> ' : "";
				if (k !== ""
						&& (j.is(":disabled") || j.parent().is(":disabled"))) {
					k = "<span>" + k + "</span>"
				}
				if (!j.data("content")) {
					n = k + '<span class="text">' + n + l + "</span>"
				}
				if (e.options.hideDisabled
						&& (j.is(":disabled") || j.parent().is(":disabled"))) {
					...
					if (j.parent().is("optgroup") && j.data("divider") != true) {
						if (j.index() == 0) {
							...
							var o = j.parent().data("subtext") !== undefined
									? '<small class="muted">'
											+ j.parent().data("subtext")
											+ "</small>"
									: "";
							var f = j.parent().data("icon") ? '<i class="'
									+ j.parent().data("icon") + '"></i> ' : "";
		...
		render : function() {
			var g = this;
			this.$element.find("option").each(function(h) {
				g.setDisabled(h, b(this).is(":disabled")
								|| b(this).parent().is(":disabled"));
				g.setSelected(h, b(this).is(":selected"))
			});
			var f = this.$element.find("option:selected").map(function(h, k) {
				var l = b(this);
				var j = l.data("icon") && g.options.showIcon
						? '<i class="glyphicon ' + l.data("icon") + '"></i> '
						: "";
				var i;
				if (g.options.showSubtext && l.attr("data-subtext")
						&& !g.multiple) {
					i = ' <small class="muted">' + l.data("subtext")
							+ "</small>"
				} else {
					i = ""
				}
			...
			if (g.multiple
					&& g.options.selectedTextFormat.indexOf("count") > -1) {
				var c = g.options.selectedTextFormat.split(">");
				var d = this.options.hideDisabled ? ":not([disabled])" : "";
				if ((c.length > 1 && f.length > c[1])
						|| (c.length == 1 && f.length >= 2)) {
					e = g.options.countSelectedText
							.replace("{0}", f.length)
							.replace(
									"{1}",
									this.$element
			...
			if (!e) {
				e = g.options.title != undefined
						? g.options.title
						: g.options.noneSelectedText
			}
		...
		liHeight : function() {
			...
			d.appendTo("body");
			var c = d.addClass("open").find(".dropdown-menu li > a")
					.outerHeight();
			d.remove();
			this.$newElement.data("liHeight", c)
		},
		setSize : function() {
			var q = this, d = this.$menu, h = d.find(".inner"), n = h
					.find("li > a"), s = this.$newElement.outerHeight(), f = this.$newElement
					.data("liHeight"), j = d.find("li .divider")
					.outerHeight(true), p = parseInt(d.css("padding-top"))
					+ parseInt(d.css("padding-bottom"))
					+ parseInt(d.css("border-top-width"))
					+ parseInt(d.css("border-bottom-width")), m = this.options.hideDisabled
					? ":not(.disabled)"
					: "", l = b(window), g = p + parseInt(d.css("margin-top"))
					+ parseInt(d.css("margin-bottom")) + 2, o, t, r, i = function() {
				t = q.$newElement.offset().top - l.scrollTop();
				r = l.height() - t - s
			};
			i();
			if (this.options.size == "auto") {
				var e = function() {
					...
					i();
					o = r - g;
					q.$newElement.toggleClass("dropup", (t > r)
									&& (o - g) < d.height()
									&& q.options.dropupAuto);
				...
				if (this.options.size && this.options.size != "auto"
						&& d.find("li" + m).length > this.options.size) {
					var k = d.find("li" + m + " > *")
							.filter(":not(.div-contain)").slice(0,
									this.options.size).last().parent().index();
					var c = d.find("li").slice(0, k + 1).find(".div-contain").length;
					o = f * this.options.size + c * j + p;
					this.$newElement.toggleClass("dropup", (t > r)
									&& o < d.height()
		...
		selectPosition : function() {
			var h = this, d = "<div />", e = b(d), g, f, c = function(i) {
				e.addClass(i.attr("class")).toggleClass("dropup",
						i.hasClass("dropup"));
				g = i.offset();
				f = i.hasClass("dropup") ? 0 : i[0].offsetHeight;
				e.css({
							top : g.top + f,
							left : g.left,
			...
			this.$newElement.on("click", function(i) {
						...
						e.toggleClass("open", !b(this).hasClass("open"));
						e.append(h.$menu)
					});
			...
			b("html").on("click", function(i) {
						if (b(i.target).closest(h.$newElement).length < 1) {
							e.removeClass("open")
						}
					})
		},
		...
		setDisabled : function(c, d) {
			if (d) {
				this.$menu.find("li").eq(c).addClass("disabled").find("a")
						.attr("href", "#").attr("tabindex", -1)
			} else {
				this.$menu.find("li").eq(c).removeClass("disabled").find("a")
		...
		clickListener : function() {
			...
			this.$menu.on("click", "li a", function(k) {
				var f = b(this).parent().index(), j = b(this).parent(), i = c.$element
						.val();
				if (c.multiple) {
					k.stopPropagation()
				}
			...
			this.$menu.on("click", "li.disabled a, li dt, li .div-contain",
					function(d) {
						d.preventDefault();
						d.stopPropagation();
						c.$button.focus()
		...
		keydown : function(o) {
			...
			if (/(38|40)/.test(o.keyCode)) {
				...
				n.each(function() {
							if (b(this).parent().is(":not(.disabled)")) {
								if (b.trim(b(this).text().toLowerCase())
										.substring(0, 1) == f[o.keyCode]) {
				...
				var r = b.trim(b(":focus").text().toLowerCase())
						.substring(0, 1);
				if (r != f[o.keyCode]) {
					k = 1;
					b(document).data("keycount", k)
			...
			if (/(13|32)/.test(o.keyCode)) {
				b(":focus").click();
				if (!l.multiple) {
					h.parent().toggleClass("open", !(o.keyCode == 32))
				} else {
					o.preventDefault()
				}
				b(document).data("keycount", 0)
	...
	b(document).data("keycount", 0).on("keydown",
			"[data-toggle=dropdown], [role=menu]", a.prototype.keydown)
}(window.jQuery);