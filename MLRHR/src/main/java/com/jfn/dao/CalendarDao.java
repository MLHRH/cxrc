...
public class CalendarDao {
	...
	private final String SQL_INSERT_calendar = "insert into calendar(start_date,end_date,jieshao) values(?,?,?)";
	...
	private final String SQL_SET_calendar_UPDATE = "update calendar set start_date=?,end_date=?,jieshao=? where id=?";
	private final static String SQL_DEL_BY_ID = "delete from calendar where id = ?";

	public boolean insert(Calendar calendar) {
		return jdbcTemplate.update(SQL_INSERT_calendar, new Object[] { calendar.getStart_date(), calendar.getEnd_date(),  calendar.getJieshao() }) == 1;
	}
...
//					calendar.setName(rs.getString("name"));
	...
	public boolean Update(Calendar calendar) {
		Object[] params = new Object[] { calendar.getStart_date(), calendar.getEnd_date(),  calendar.getJieshao(), calendar.getId() };
