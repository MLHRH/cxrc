...
public class JcqnDoc01Dao {
	...
	private final String SQL_SET_UPDATE = "update jcqn_doc01 set user_id=?,name=?,sex=?,minzu=?,birthday=?,title=?,document_type=?,document_number=?,direction=?,declaration_area=?,telephone=?,fax,mobile_phone=?,email=?,zip_code=?,addresses=?,study_country=?,study_colleges=?,"
			+ "study_major=?,study_education=?,study_start_time=?,study_end_time=?,work_country=?,work_company=?,work_position=?,work_start_time=?,work_end_time=?,journal_title=?,organization_position=?,organization_term=?,unit_name=?,organization_code=?,legal_representative=?,"
...
//	private final String SQL_QUERY_EXISTED = "SELECT COUNT(id) from jcqn_doc01 where user_id = ?";
	...
	//根据USERID查询
	public List<JcqnDoc01> getByUserId( int user_id )
	{
		return jdbcTemplate.query( SQL_Get_BY_USERID, new Object[]{ user_id},
				new ResultSetExtractor<List<JcqnDoc01>>()
				{
			@Override
			public List<JcqnDoc01> extractData( ResultSet rs )
					throws SQLException, DataAccessException
					{
				int i=0;
				List<JcqnDoc01> list = new ArrayList<JcqnDoc01>();
				while( rs.next() )
				{   
